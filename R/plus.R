...
## with k1 = k2 at the beginning and end, k2 = k1+1 otherwise
 ...
 lam <- c(lam,-1)
 j <- length(lam.path)
 lam.path <- c(lam.path, -1)
 j.ind <- 1:(j+1)
 j.set <- c(rep(TRUE,j),FALSE)
 k.ind <- 1:(k+1)
 k1 <- rep(1,k+1)
 k2 <- k1
 w1 <- k1
 m1 <- sum(lam >= lam.path[1]) +1
 m <- m1 
 while ((m <= k) & any(j.set)) {
   thresh <- min(lam[m], lam.path[ min(j.ind[j.set]) ])
   j.set[ lam.path > thresh ] <- FALSE
   if (any(j.set)) {
     thresh <- lam.path[ min(j.ind[j.set]) ]
     k2[ (k.ind >= m) & (lam >= thresh) ] <- min(j.ind[j.set])
     k1[ (k.ind >= m) & (lam >= thresh) ] <- min(j.ind[j.set]) - 1
     m <- m + sum((k.ind >= m) & (lam >= thresh))
   }
 }
 m2 <- m-1
 w1[m1:m2] <- (lam[m1:m2] - lam.path[k2[m1:m2]])/(lam.path[k1[m1:m2]] - lam.path[k2[m1:m2]])
 if (m<= k) {
   k1[m:k] <- 0
   k2[m:k] <- 0
 } 
 if (m2==0) return(list(total.hits=m2))
 else return(list(k1=k1[1:m2],k2=k2[1:m2],w1=w1[1:m2], total.hits=m2))
}
